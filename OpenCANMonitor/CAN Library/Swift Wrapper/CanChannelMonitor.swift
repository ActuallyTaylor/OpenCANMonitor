...
class Canbus {
    ...
    init(bus: USBBus, baudRate: BaudRate) throws {
...
extension CanChannelMonitor {
    func load() throws {
        ...
        let (response, url) = showOpenPanel()
        
        guard response != .cancel else {
            throw MonitorError.canceled
        }
    ...
    func showOpenPanel() -> (NSApplication.ModalResponse, URL?) {
